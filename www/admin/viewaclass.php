...
<li class="nav-item" role="presentation"><a class="nav-link" href="userman/viewusers.php"><i
    ...
    <div class="d-flex flex-column" id="content-wrapper">
        <div id="content">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            if ($_SESSION['user_level'] == 'user') {
                                ...
                                echo "<a href='#' onclick='delete_class({$id})'  class='btn btn-danger'>Delete</a>";
                            }
                            ?>
                        </td>
                    </tr>
            ...
            <div class="container-fluid">
                <?php
                    if ($_SESSION['user_level'] == 'instructor' or $_SESSION['user_level'] == 'admin') {

                        echo "<h3 class='text-dark mb-1' > Who's Booked On </h3>";

                        $id = isset($_GET['id']) ? $_GET['id'] : die('ERROR: Record ID not found.');

                        include '../../dbconnect.php';
                        try {
                            $sql = "SELECT * FROM bookings JOIN users ON bookings.userid = users.id JOIN classes ON bookings.classid = classes.id WHERE classid = ?";
                            $stmt = $db->prepare($sql);
                            $stmt->bind_param("i", $id);
                            $stmt->execute();
                            $result = $stmt->get_result();
